...
export default function AttendeeDashboard({ account }) {
  const [tab, setTab] = useState('book');
  return (
    <div className="space-y-8">
      <div className="flex gap-4 p-1 bg-white/5 border border-white/10 rounded-xl w-fit">
        <button onClick={() => setTab('book')} className={`px-6 py-2 rounded-lg ${tab === 'book' ? 'bg-cyan-500 text-black font-bold' : ''}`}>Book Tickets</button>
        <button onClick={() => setTab('tickets')} className={`px-6 py-2 rounded-lg ${tab === 'tickets' ? 'bg-cyan-500 text-black font-bold' : ''}`}>My Tickets</button>
      </div>
      {tab === 'book' ? <BookTicket /> : <MyTickets account={account} />}
    </div>
  );
}